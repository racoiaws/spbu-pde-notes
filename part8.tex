...
А минимум существует по предыдущей теореме. Значит решение уравнения Пуассона  существует. А что же с единственностью?
...
Докажем единственность. Нам понадобятся две вспомогательные леммы.
...
\begin{note} Попутно доказали равносильность задач! Если $f$ из $\m{L2Om}$, $u$ из $\m{H01Om}$, то
...
\subsection{Использование теоремы Рисса}
...
Значит, введённая норма эквивалентна старой, и $\widehat{H}$ --- сепарабельное гильбертово пространство. В то же время, вложение $\widehat{H} \hookrightarrow H$ непрерывно:
$$ \norm*{H}{u} = \norm*{2}{u} \leq C \norm*{2}{\nabla u} = C \norm*{\widehat{H}}{u}.$$
